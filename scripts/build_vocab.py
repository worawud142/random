...
THAI_GLYPH_FIXES = {
    ...
    "พลบค่า": "พลบค่ำ", "พลบค่่า": "พลบค่ำ", "แม่นย่า": "แม่นยำ", "ย่าเกรง": "ยำเกรง",
    "ร่ามะนา": "รำมะนา", "ล่าธาร": "ลำธาร", "ล่าพูน": "ลำพูน",
    "ล่าสัน": "ล่ำสัน", "ล่่าสัน": "ล่ำสัน", "สรุ่ยสุร่าย": "สุรุ่ยสุร่าย", "ส่าเนียง": "สำเนียง",
    "ส่ารวจ": "สำรวจ", "เสนาอ่านาตย์": "เสนาอำมาตย์", "ก่านัล": "กำนัล",
    "คร่าคร่า": "คร่ำคร่า", "คร่่าคร่า": "คร่ำคร่า", "คร่่าครวญ": "คร่ำครวญ",
    "ค่าประพันธ์": "คำประพันธ์", "จ่านรรจ์": "จำนรรจ์",
    ...
    "เพาะช่า": "เพาะชำ", "ละล่าละลัก": "ละล่ำละลัก", "ละล่่าละลัก": "ละล่ำละลัก",
    "ล่าน้่าเหมือง": "ลำน้ำเหมือง", "สม่่าเสมอ": "สม่ำเสมอ", "ย่่ายี": "ย่ำยี",
...
ENGLISH_MEANING_FIXES = {
    ...
    "คุกกี": "คุกกี้", "วันนี": "วันนี้", "นิ วมือ": "นิ้วมือ", "เสื อแขนยาว": "เสื้อแขนยาว",
    ...
    "เสื อยืด": "เสื้อยืด", "ชั นบน": "ชั้นบน", "ฤดูผน, ผนตก": "ฤดูฝน, ฝนตก",
    "ทิศทาง / ค้าแนะน้า": "ทิศทาง / คำแนะนำ",
...
def numbered_segments(line: str, value_start: str):
    # A number is a row number only when the following value begins in the
    # expected alphabet. This keeps numbers inside meanings (such as เลข 11)
    # attached to their vocabulary entry.
    matches = list(re.finditer(rf"(?<!\S)(\d{{1,4}})\s+(?={value_start})", line))
...
def read_thai(grade: int):
    ...
    for line in text.splitlines():
        for number, value in numbered_segments(line, r"[ก-๙]"):
            is_document_header = (
...
def read_english(grade: int):
    ...
    words = {}
    for line in text.splitlines():
        for number, value in numbered_segments(line, r"[A-Za-z]"):
